...
def process_frame(frame, alert_flags, last_snapshot_time, snapshot_interval=10, frame_number=0, video_writer=None):
    ...
    if datetime.now() >= last_snapshot_time + timedelta(seconds=snapshot_interval):
        snapshot_filename = f"snapshot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
        cv2.imwrite(snapshot_filename, frame)
        log_alert(f"Snapshot taken and saved as {snapshot_filename}")
        last_snapshot_time = datetime.now()

    # Save the frame to the video file if the writer is initialized
    if video_writer is not None:
        video_writer.write(frame)

    return frame, alert_flags, last_snapshot_time, video_writer

# Function to capture webcam feed and process frames
def capture_and_process():
    cap = cv2.VideoCapture(0)  # Open the webcam

    # Get screen width and height for full HD display (1920x1080)
    screen_width = 1920
    screen_height = 1080
    ...
    while cap.isOpened():
        ...
        frame, alert_flags, last_snapshot_time, video_writer = process_frame(
            frame, alert_flags, last_snapshot_time, snapshot_interval, frame_number, video_writer)

        cv2.imshow("Gender Detection and SOS Monitoring", frame)
...
# Run the webcam feed and process it
